...
BOTS_AND_TEST_MESSAGE["CmdLine"] =  ProtocolMessage(
...
BOTS_AND_TEST_MESSAGE["LinkAwareBot"] =  ProtocolMessage(
    role="user", content="What is the difference between https://arxiv.org/pdf/2201.11903.pdf and https://arxiv.org/pdf/2305.10601.pdf"
)
...
BOTS_AND_TEST_MESSAGE["tiktoken"] =  ProtocolMessage(
...
async def test_bot(bot_name, message):
    print(bot_name)
    print(message)
    async for partial in get_bot_response(messages=[message], bot_name=bot_name, api_key=os.environ["POE_API_KEY"]):
        print(partial)
